-- ==============================================================================
-- FICHIER : SCHEMA_STRUCTURE_V3.5_CLEAN.sql
-- Architecture ERP V3.5 - Version nettoyée (UTF-8 garanti)
-- ==============================================================================

-- Connexion et (re)création de la base
\c postgres;
...
-- -----------------------------------------------------------------------------
-- 1. NETTOYAGE COMPLET (supprime tout dans l'ordre inverse des dépendances)
-- -----------------------------------------------------------------------------
...
CREATE TABLE entreprise (
    ...
    nom             VARCHAR(200) NOT NULL,
    groupe_id       INTEGER,
    type_entreprise VARCHAR(20) NOT NULL 
        CHECK (type_entreprise IN ('CLIENT', 'FOURNISSEUR', 'INTERNE', 'PARTENAIRE')),
...
CREATE TABLE proforma_demande_achat_details (
    id                        SERIAL PRIMARY KEY,
    proforma_demande_achat_id INTEGER NOT NULL,
    article_id                INTEGER NOT NULL,
...
-- Fin du script
 ...
 SET valeur_stock_total = (
     ...
     WHERE ls.article_id = s.article_id
       ...
       AND ls.statut = 'ACTIF'
 ) WHERE s.methode_valorisation_stock_id IN (2, 3);  -- FIFO et LIFO