                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-col">
                        <div class="flex items-center gap-3">
                            <h1 class="text-xl font-bold text-slate-800 tracking-tight">Détail du Paiement</h1>
                    ...
                    <div class="flex flex-col h-full justify-between">
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Montant du
                                règlement</p>
                            <div class="text-3xl font-bold text-indigo-600 tracking-tight">
                ...
                <div class="modern-card p-6 md:col-span-2">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                        <div class="detail-group">
                            <span class="detail-label">Facture associée</span>
                            <span class="detail-value font-mono text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded w-fit">
                                {{ paiement.numero_facture || '#' + paiement.facture_vente_id }}
                            </span>
                        </div>
                        ...
                        <div
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    <div
                                        class="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 text-xs font-bold">
                                        {{ (personnelFullName(paiement.caisse_mouvement)[0] || 'U').toUpperCase() }}
                                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative">
                            ...
                            <select v-model="editModel.mode_paiement_id" class="modern-select pl-11">
                    ...
                    <div class="form-group">
                        ...
                        <div class="relative">
                            ...
                            <input v-model="editModel.date_paiement" type="date" class="modern-input pl-11" />
                        </div>
                    </div>
                    ...
                    <!-- N° Reçu -->
                    <div class="form-group">
                        <label class="label">Numéro de Reçu (Interne)</label>
            ...
            <div v-if="showValidate && !paiement.caisse_mouvement" class="modern-card p-6 md:p-8 bg-slate-50 border-slate-200">
                <div class="flex items-start justify-between mb-6">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 mt-1">Cette action va générer une écriture comptable dans la
                            caisse
                            sélectionnée.</p>
                    </div>
                ...
                <div
                    ...
                    <div class="form-group md:col-span-2">
                        <label class="label required">Sélectionner la Caisse</label>
                        <div class="relative">
                            <div class="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
                                <Wallet class="w-4 h-4 text-slate-400" />
                            </div>
                            <select v-model="validatePayload.caisse_id" class="modern-select pl-11">
                    ...
                    <div class="form-group">
                        ...
                        <input v-model="validatePayload.libelle" class="modern-input"
                            placeholder="Encaissement facture..." />
                    </div>

                    <!-- Personnel (Souvent auto-rempli, peut être caché ou readonly) -->
                    <div class="form-group">
                        ...
                        <div class="relative">
                            ...
                            <input :value="currentUser?.nom || 'Utilisateur connecté'" readonly
                                class="modern-input pl-11 bg-slate-100 text-slate-500 cursor-not-allowed" />
                        </div>
                    </div>
                </div>
...
const load = async () => {
    try {
        const resp = await paiementVenteService.getById(route.params.id)
...
const loadFilters = async () => {
    try {
        const resp = await paiementVenteService.filters() // Assurez-vous que cette méthode existe dans votre service ou adaptez-la
        modes.value = resp.data?.modes || []
        // Fallback si l'API filters n'existe pas, charger via modePaiementService si besoin
...
const cancelEdit = () => {
    editing.value = false
    // Reset model is handled by next load or manually here if preferred, 
    // currently we just hide and reload data would be safer but keeping UI state:
    load()
}

const saveEdit = async () => {
    try {
        await paiementVenteService.update(paiement.value.id, editModel.value)
        // alert('Modifications enregistrées') // Pas d'alert, UX fluide
        editing.value = false
        await load()
    } catch (err) {
        console.error(err)
...
const openValidate = async () => {
    ...
    if (paiement.value.caisse_mouvement) {
        ...
        let success = false
        try {
            console.log('Validating paiement (direct) id=', paiement.value.id)
            await paiementVenteService.validate(paiement.value.id, { user_id: currentUser.value.id })
        ...
        if (success) router.push({ name: 'paiements-vente' })
        return
    }

    validatePayload.value.personnel_id = currentUser.value.id || null
    // Pré-remplir le libellé pour gagner du temps
    validatePayload.value.libelle = `Encaissement Facture ${paiement.value.numero_facture || ''}`
...
const doValidate = async () => {
    ...
    validating.value = true
    try {
        console.log('Validating paiement with payload:', validatePayload.value)
        await paiementVenteService.validate(paiement.value.id, Object.assign({}, validatePayload.value, { user_id: currentUser.value.id }))
        showValidate.value = false
        await load() // Recharger pour voir le statut validé
    } catch (err) {
        console.error(err)
        alert(err.response?.data?.error || 'Erreur lors de la validation')
    } finally {
...
const goBack = () => {
    router.push({ name: 'paiements-vente' }) // Assurez-vous que la route est correcte
}
...
const exportDetailPdf = () => {
    ...
    window.open(`${base}/paiements/vente/${route.params.id}/export${params.toString() ? ('?' + params.toString()) : ''}`, '_blank')
}
...
onMounted(() => {
    ...
    // If we were navigated here from the list validate action, open validation mode
    if (route.query && route.query.showValidate === '1') {
        validatePayload.value.personnel_id = currentUser.value.id || null
        validatePayload.value.libelle = `Encaissement Facture ${paiement.value.numero_facture || ''}`
        showValidate.value = true
    }
